...
function slideLeft(){
	for(let r=0; r<rows; r++){
		...
		for(let c=0; c<columns; c++){
			...
			// This is line for animation
			// if current tile != to the original tile, apply animations value
...
function hasEmptyTile(){
    for(let r=0; r<rows; r++){
        for(let c=0; c<columns; c++){
            // Check if current tile is 0, if yes it will return true.
            if(board[r][c] == 0){
                return true;
            }
        }
...
function setTwo(){
    ...
    while(!found){
        // Math.random() - generates a random number based on the given condition.
...
function hasLost(){
    for(let r=0; r<rows; r++){
        for(let c=0; c<columns; c++){
            // found an empty tile, user hasn't lost.
            if(board[r][c] == 0){
                return false;
            }
            ...
            // Found adjacent cell with the same value, user hasn't lost.
            return false;
            }
        }
    }
...
// event listener to capture touch in the screen and assigns the x and y coordinates in the startX and startY variables.
...
document.addEventListener("touchend", (e) => {
	...
	// check if the touch was horizontal or vertical.
	// horizontal > vertical
